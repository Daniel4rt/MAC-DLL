...
static bool mmo_auth_fromsql(AccountDB_SQL* db, struct mmo_account* acc, uint32 account_id) {
	...
	Sql_GetData(sql_handle, 16, &data, NULL); safestrncpy(acc->mac, data, sizeof(acc->mac));//Oboro NoCheats' lets use it everytime
#ifdef VIP_ENABLE
...
static bool mmo_auth_tosql(AccountDB_SQL* db, const struct mmo_account* acc, bool is_new) {
	...
	{// insert into account table
		...
		||  SQL_SUCCESS != SqlStmt_BindParam(stmt, 16, SQLDT_STRING,    (void*)&acc->mac,	      strlen(acc->mac)) //Oboro NoCheats
#ifdef VIP_ENABLE
	...
	{// update account table
		...
		||  SQL_SUCCESS != SqlStmt_BindParam(stmt, 15, SQLDT_STRING,    (void*)&acc->mac,	      strlen(acc->mac)) //Oboro NoCheats
#ifdef VIP_ENABLE
